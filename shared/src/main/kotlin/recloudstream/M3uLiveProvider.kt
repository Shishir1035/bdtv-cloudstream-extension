...
/**
 * Generic M3U live-stream provider. One implementation shared by every module.
 *
 * Per-module identity comes from BuildConfig.MODULE_KEY (= the gradle project/dir name,
 * injected in the root build.gradle.kts). That key also selects this module's playlist
 * URL inside the remote config JSON ({ "<key>": "<playlist-url>", ... }, CONFIG_JSON secret).
 */
class M3uLiveProvider : MainAPI() {
    // This module's key inside the remote config JSON, and its display name.
    private val key = BuildConfig.MODULE_KEY

    override var name = key
    override var mainUrl = "config://$key"
    ...
    // One config entry: { link, desc, logo }. Only `link` is needed at runtime.
    private data class ExtConfig(
    ...
    private fun playlistUrl(): String? = try {
        parseJson<Map<String, ExtConfig>>(BuildConfig.CONFIG_JSON)[key]?.link?.takeIf { it.startsWith("http") }
    } catch (_: Exception) {
        null
    }
